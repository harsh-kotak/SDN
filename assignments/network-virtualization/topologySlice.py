...
class TopologySlice (EventMixin):
    ...
    def Slice(self, in_port, out_port):
        msg = of.ofp_flow_mod()
        match = of.ofp_match()
    ...
    def _handle_ConnectionUp(self, event):
        ...
        if dpid == "00-00-00-00-00-01":
            event.connection.send(self.Slice(1, 3))
            event.connection.send(self.Slice(3, 1))
            event.connection.send(self.Slice(2, 4))
            event.connection.send(self.Slice(4, 2))
           # log.debug("Adding slicing flows for switch %s.", dpid)

        elif dpid == "00-00-00-00-00-02":
            event.connection.send(self.Slice(1, 2))
            event.connection.send(self.Slice(2, 1))
